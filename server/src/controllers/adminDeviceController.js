...
const { subscribeDevice } = require("../services/mqttSubscription");

// =====================================================
// ADMIN DEVICE CONTROLLER
//
// Handles device management for the admin role. These
// endpoints return and operate on all devices.
// =====================================================
...
exports.getAdminDeviceDetails = async (req, res) => {
  try {
    ...
    // 1. Get the device
    const [devices] = await pool.execute(`SELECT * FROM devices WHERE id = ?`, [
      id,
    ]);
...
// account owner involved in the flow.
// =====================================================

exports.registerDevice = async (req, res) => {
  console.log("ADMIN DEVICE REGISTRATION BODY:", req.body);

  const { imei, solution, deviceVersion } = req.body;
  ...
  try {
    ...
    await pool.execute(
      `
            ...
            VALUES (?, ?, ?, ?)
            `,
      [imei, solution, deviceVersion, mqttPassword],
    );

    console.log(`Device ${imei} registered successfully by admin`);
    ...
    try {
      subscribeDevice(imei, solution, deviceVersion);
    } catch (mqttErr) {
      console.error("MQTT subscription failed:", mqttErr);
    ...
    console.error("Admin device registration error:", err);
