...
export default function AdminDeviceDetails() {
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      try {
        ...
        console.log("ADMIN DEVICE DETAILS:", res.status, raw);
        ...
        if (!res.ok) {
          throw new Error(
            result.error ||
              result.message ||
              `Failed to load device details (${res.status})`,
          );
        }
  ...
  if (loading) {
    return (
      <Page>
        <AdminDevicePageHeader
          title="DEVICE DETAILS"
          backPath="/admin/devices"
        />

        <Message>Loading device details...</Message>
      </Page>
    );
  }
  ...
  if (!details?.device) {
    return (
      <Page>
        ...
        <Message>No device found</Message>
      </Page>
    );
  }
  ...
  const lastUpdated = latest?.created_at ? new Date(latest.created_at) : null;
      ...
      >
        ...
        >
          ...
          >
            <DetailRow
              label="DEVICE NO."
              value={device.imei || device.device_id || device.id || "--"}
            />

            <DetailRow
              label="DEVICE TYPE"
              value={device.solution || device.device_type || "--"}
            />
