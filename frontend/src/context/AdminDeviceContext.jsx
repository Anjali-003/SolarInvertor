...
//
// Device context for the admin dashboard:
//   - reads "adminToken"
//   - calls /api/admin/...
//   - persists the selected device under "selectedAdminDevice"
// =====================================================
...
export function AdminDeviceProvider({ children }) {
  const [selectedDevice, setSelectedDevice] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("selectedAdminDevice") || "null");
    } catch (error) {
      console.error("Failed to read selected admin device:", error);
  ...
  const fetchLatest = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(json?.message || "Failed to fetch latest device data");
      }
      ...
      if (createdAt) {
        ...
        if (!Number.isNaN(parsedDate.getTime())) {
          ...
          console.error("Invalid created_at received from API:", createdAt);
          setLastUpdated(null);
        }
      } else {
        setLastUpdated(null);
      ...
      console.error("Admin latest device fetch failed:", err);
  ...
  const fetchEnergy = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          json?.error || json?.message || "Failed to fetch admin energy",
        );
      }
      ...
      console.error("Admin energy fetch failed:", err);
  ...
  const fetchEnergyChart = useCallback(
    async (period, offset = 0) => {
      ...
      try {
        ...
        if (!res.ok) {
          console.error("Admin chart API error:", result);
        ...
        console.error("Admin chart fetch failed:", err);
  ...
  }, [selectedDevice, fetchLatest, fetchEnergy]);
  ...
  const handleSetSelectedDevice = useCallback((device) => {
    ...
    if (device) {
      localStorage.setItem("selectedAdminDevice", JSON.stringify(device));
    } else {
      localStorage.removeItem("selectedAdminDevice");
    }
...
export function useAdminDevice() {
  ...
  if (!context) {
    throw new Error("useAdminDevice must be used inside AdminDeviceProvider");
  }
